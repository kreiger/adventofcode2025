...
my $size = @boxes < 1000 ? 10 : 1000;
@connections = splice @connections, 0, $size;
...
@circuits = sort { @{$b->{boxes}} <=> @{$a->{boxes}} } @circuits;

@circuits = splice @circuits, 0, 3;

say product map { scalar @{$_->{boxes}} } @circuits;
